...
class EmailEvent extends Controller{ 
	 public function email($to, $title, $content){
        $mail = new MySendMail();
        // 1qqftp   @@@2发件人账号  @@@3授权码在邮箱设置里可以找到
//        $mail->setServer("smtp.163.com", "kjhz0507@163.com", "kjhz0507", 465, true);
        $mail->setServer("smtp.qq.com", "462495183@qq.com", "qqkxowdfacmqbjgc", 465, true);//1、SMTP服务器  2、发件人邮箱  3、授权码  4、端口
        $mail->setFrom("462495183@qq.com");
        $mail->setReceiver($to);
        $mail->setDebug(false);
...
class MySendMail {
    ...
    */
    public function sendMail() {
        ...
        foreach ($command as $value) {
            $result = $this->_isSecurity ? $this->sendCommandSecurity($value[0], $value[1],$this->Debug) : $this->sendCommand($value[0], $value[1],$this->Debug);
            if($result) {
                continue;
            }
            else{
    ...
    * @return boolean
    */
    protected function sendCommand($command, $code,$debug) {
        if($debug) echo 'Send command:' . $command . ',expected code:' . $code . '<br />';
        ...
        try{
            if(socket_write($this->_socket, $command, strlen($command))){
                ...
                if($debug) echo 'response:' . $data . '<br /><br />';
                if($data) {
                    $pattern = "/^".$code."+?/";
    ...
    * @return boolean
    */
    protected function sendCommandSecurity($command, $code,$debug) {
        if($debug) echo 'Send command:' . $command . ',expected code:' . $code . '<br />';
        try {
