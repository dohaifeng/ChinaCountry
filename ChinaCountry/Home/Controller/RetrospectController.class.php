...
/**
 * Description of RetrospectController
 *
 * @author feng
 */
...
class RetrospectController extends BaseController {

    public function index() {
        //分页加查询
        $where = array("column_id" => 2);
        $grade = M("video");
        ...
        $show = $page->show();
        $demand = $grade->where($where)->order('id desc')->field('id,title,date,thumbnail,introduce')->limit($page->firstRow . ',' . $page->listRows)->select();
        foreach ($demand as $key => $value) {
            $demand[$key]['introduce'] = htmlspecialchars_decode($value['introduce']);
        }
